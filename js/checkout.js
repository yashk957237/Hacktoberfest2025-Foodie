(function () {
    ...
    function renderSuggestions(items, query) {
      ...
      suggestionsEl.innerHTML = results
        .map((it, idx) => {
          ...
          return `<button type="button" role="option" class="autocomplete-item" data-idx="${idx}">${label}</button>`;
        })
        .join('');
  ...
  function initPincodeValidation() {
    ...
    const lookupPin = debounce(async (pin) => {
      ...
      try {
        ...
        const cityFromPin =
          postOffices[0].District || postOffices[0].Division || postOffices[0].Region || '';

        if (cityInput && !cityInput.value.trim() && cityFromPin) {
          cityInput.value = cityFromPin;
        }
  ...
  // Expose for checkout page to call
